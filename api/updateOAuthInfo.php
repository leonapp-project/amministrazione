...
The access_to, which is a JSON, should have the key/permission administration.OAuth.updateInfo set to true or
...
if (!isset($access_to['administration.OAuth.updateInfo']) && !isset($access_to['administration.OAuth.*']) && !isset($access_to['administration.*']) && !isset($access_to['*'])) {
    //the OAuth token is not valid
    echo json_encode(array("exit" => "error", "error" => "OAuth token not valid"));
    exit();
}
...
$stmt = $mysqli->prepare("SELECT * FROM OAuth WHERE id = ?");
...
if ($field != "type" && $field != "grade" && $field != "okey" && $field != "expiration" && $field != "access_to" && $field != "commento") {
...
//update the OAuth info
$stmt = $mysqli->prepare("UPDATE OAuth SET $field = ? WHERE id = ?");
$stmt->bind_param("si", $value, $id);
$stmt->execute();
//check if the update was successful
if ($stmt->affected_rows === 0) {
    //the update was not successful
    echo json_encode(array("exit" => "error", "error" => "update not successful"));
    exit();
}
//the update was successful
echo json_encode(array("exit" => "success", "message" => "OAuth info updated"));
exit();
?>